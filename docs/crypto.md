...
| `bip39.ts` | Mnemonic generation and seed derivation via PBKDF2-SHA512 (2048 iterations, salt `"mnemonic" + passphrase`). `generateMnemonic(strength)` defaults to 256-bit entropy (24 words), matching what `generateNewAccount` in `lib/accounts.ts` requests so that post-quantum keys can later be derived from the same seed without the seed being the weakest link. Both 12- and 24-word phrases remain valid for import. |
