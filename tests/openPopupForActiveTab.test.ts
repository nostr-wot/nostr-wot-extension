...
test('matches when the active tab origin equals the origin', () => {
  assert.equal(originMatchesActiveTab('https://example.com/some/path', 'https://example.com'), true);
  assert.equal(originMatchesActiveTab('https://example.com', 'https://example.com'), true);
  assert.equal(originMatchesActiveTab('http://localhost:3000/app', 'http://localhost:3000'), true);
});

test('does NOT match a different active-tab host (inactive/other tab requesting)', () => {
  assert.equal(originMatchesActiveTab('https://other.com', 'https://example.com'), false);
});

test('subdomain of the origin does not match the bare origin', () => {
  assert.equal(originMatchesActiveTab('https://sub.example.com', 'https://example.com'), false);
});

test('no active tab url -> false (e.g. no active tab / new-tab page)', () => {
  assert.equal(originMatchesActiveTab(undefined, 'https://example.com'), false);
  assert.equal(originMatchesActiveTab(null, 'https://example.com'), false);
...
test('malformed active-tab url -> false (no throw)', () => {
  assert.equal(originMatchesActiveTab('not a url', 'https://example.com'), false);
});
...
// yet, the active tab arrives with url: undefined, the origin comparison fails
...
test('matches when the requesting tab IS the active tab, even with no visible URL', () => {
  assert.equal(requestIsFromActiveTab({ id: 7, url: undefined }, 'https://example.com', 7), true);
});

test('does NOT match when the request came from a different tab', () => {
  assert.equal(requestIsFromActiveTab({ id: 7, url: undefined }, 'https://example.com', 9), false);
});
...
test('a known tab id decides it — a mismatched URL cannot override it', () => {
  ...
  assert.equal(requestIsFromActiveTab({ id: 7, url: 'https://other.com' }, 'https://example.com', 7), true);
  // Different tab, and the URL happens to agree. Still not the requesting tab.
  assert.equal(requestIsFromActiveTab({ id: 9, url: 'https://example.com' }, 'https://example.com', 7), false);
});

test('falls back to the origin when no tab id is available', () => {
  assert.equal(requestIsFromActiveTab({ id: 7, url: 'https://example.com' }, 'https://example.com', undefined), true);
  assert.equal(requestIsFromActiveTab({ id: 7, url: 'https://other.com' }, 'https://example.com', undefined), false);
  // No id and no readable URL: nothing to verify, so stay closed.
  assert.equal(requestIsFromActiveTab({ id: 7, url: undefined }, 'https://example.com', undefined), false);
});

test('no active tab at all -> false', () => {
  assert.equal(requestIsFromActiveTab(undefined, 'https://example.com', 7), false);
});
